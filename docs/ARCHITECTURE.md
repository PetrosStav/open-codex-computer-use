...
- Windows runtime 默认只连接已经运行的 app，不会在 `get_app_state` 找不到进程时自动 `Start-Process`，也不会默认允许 `SetFocus` secondary action；这两条前台抢占路径分别需要 `OPEN_COMPUTER_USE_WINDOWS_ALLOW_APP_LAUNCH=1` 和 `OPEN_COMPUTER_USE_WINDOWS_ALLOW_FOCUS_ACTIONS=1` 显式打开。runtime 在读取 UIA 前启用 per-monitor DPI awareness，确保 UIA bounds、Win32 hit testing、截图和 cursor placement 都使用 physical screen coordinates。每次 snapshot 和 action 都会在目标 PID 内优先复用当前前台窗口；主窗口被 modal 禁用时，则通过 Win32 顶层窗口枚举选择可见且 enabled 的 dialog，因此 common file dialog 和 nested error dialog 不会被 `MainWindowHandle` 重新夺走焦点。`type_text` 默认优先对可写文本控件的 child HWND 发送 `EM_REPLACESEL`；focused 控件保留当前 selection/caret，后台控件才先用 `EM_SETSEL` 移到末尾，不再默认走可能触发前台激活的 UIA `ValuePattern.SetValue` fallback；需要旧行为时必须设置 `OPEN_COMPUTER_USE_WINDOWS_ALLOW_UIA_TEXT_FALLBACK=1`。`press_key` 和 physical pointer action 是例外：它们先激活目标 app 的 active window，再发送真实 system input；如果 Windows 拒绝激活、目标没有保持前台或 pointer point 不再命中目标 top-level HWND，它们会明确失败，避免输入落到其他 app。`press_key` 通过 `SendInput` 投递完整的 key-down/key-up 序列，Home / End / arrow / Page / Insert / Delete / Windows 等 extended keys 会设置 `KEYEVENTF_EXTENDEDKEY`。physical pointer layer 通过 `SetCursorPos` 移动真实鼠标，使用 `SendInput` 投递 left / right / middle button 和 vertical / horizontal wheel state，并在需要时 best-effort 聚焦 point 下的 child HWND；click count 限制为 `1..10`，drag path 最多接受 `128` 个中间点且整条路径最多生成 `600` 个插值 step。button-down 前和 drag 移动过程中都会重新验证目标仍在前台，held button 一律在 `finally` 中释放，单个 tool call 之外不保留 down state。只有目标已最小化时才恢复窗口，避免破坏 maximized window 的坐标映射。这使 Paint / WinUI canvas 能在第一次动作时收到真实 drag，也让无 UIA action 的 canvas / web surface 获得通用 foreground fallback，但这些 action 会暂时抢前台并移动用户指针。UIA pattern 和 Win32 message fallback 本身仍是 best-effort：很多控件可以在后台响应，但 Windows 没有一套对所有 GUI toolkit 都等价于 macOS AX 的后台键鼠输入模型。
- 这 10 个 tool 的协议面与 macOS 主线基本一致：`list_apps`、`get_app_state`、`click`、`hover`、`perform_secondary_action`、`scroll`、`drag`、`type_text`、`press_key`、`set_value`。其中 element-targeted action 会优先复用上一轮 `get_app_state` 的 runtime id / automation metadata，coordinate action 使用 screenshot/window-relative 坐标。Windows 新增的 `hover` 接受 element 或 coordinate target，并明确移动真实 pointer；`drag` 继续接受 straight-line `from_x/from_y` 到 `to_x/to_y`，也可以通过 `path` 增加中间点并用 `mouse_button` 选择 left / right / middle。
- Windows `click_method=accessibility` 映射到 UI Automation pattern，`app_post` 映射到 HWND window message，`global` 显式请求 foreground physical input；macOS-only 的 `sky_click` 仍在 snapshot lookup 前明确返回 unsupported。`auto` 只对 single left click 尝试 UIA / MSAA semantic activation，pattern 不存在、调用失败、使用其他 mouse button 或 click count 大于一时自动 fallback 到 physical pointer input。`scroll` 同样先尝试 UIA `ScrollPattern`，不存在或执行失败时从 live UIA bounds 解析 target point 并发送 physical wheel input。Windows common file dialog 会把 Save / Cancel / overwrite confirmation 等原生 `Button` / `CCPushButton` child HWND 错报成没有 `InvokePattern` 的 `Pane`；snapshot 会把这类节点规范化成可 Invoke 的 button，click 则通过异步 `BM_CLICK` 激活，避免 Save 打开 nested confirmation 时同步 window message 阻塞 MCP call。文件对话框的 `SysTreeView32` navigation pane 在 UIA 下还是空树，因此 snapshot 会通过 MSAA `IAccessible` 补入当前可见的 Desktop / Downloads / Documents 等 tree item，click / Invoke 调用对应 child 的 default `Navigate` action。其他 element-targeted message fallback 也优先投递给元素自己的 child HWND，而不是错误地把 child-local 点击发给 dialog root。
