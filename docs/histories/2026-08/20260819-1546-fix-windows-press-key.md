...
### Design Intent (Why)
`PostMessage` does not update Windows keyboard state, so modifier combinations degrade in applications that query real key state. It can also produce incorrect printable-key behavior in Chromium. `SendInput` follows the normal keyboard pipeline, but it must target the foreground app; thread attachment makes that activation reliable while foreground verification prevents input from reaching the wrong application. Native dialogs disable the process's main window and use a different HWND, so resolving the active same-process window is required before every snapshot and action. Native button controls also require child-HWND dispatch because posting parent-window mouse messages does not route them to the control under the coordinates. Common-dialog navigation destinations require an MSAA fallback because their `SysTreeView32` provider exposes no UIA children. Modern canvases similarly ignore root-HWND mouse messages, so drag must use foreground system input and preserve window geometry after snapshot coordinates are captured.
